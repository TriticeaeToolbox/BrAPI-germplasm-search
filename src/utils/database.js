 ...
 * @param  {Function} callback   Callback function(err, cache_address, cache_params, cache_count, term_count)
 */
function getDBTerms(database, force, progress, callback) {
    ...
    // Check Parameters
    if ( !database ) {
        return callback("BrAPI database properties not provided");
    }
    else if ( !database.address || typeof database.address !== 'string' ) {
        return callback("BrAPI database address is required");
    }
    else if ( typeof callback !== 'function' ) {
        return callback("callback must be a function");
    }
    else if ( progress && typeof progress !== 'function' ) {
        return callback("progress must be a function");
    }
    else if ( force && typeof force !== 'boolean' ) {
        return callback("force must be a boolean set to true or false");
    }

    // Create BrAPI Node
    let brapi = new BrAPI(database.address, database.version, database.auth_token, database.call_limit);
    let brapi2 = new BrAPI(database.address.replace(/\/v1\/?$/, '/v2'), 'v2', database.auth_token, database.call_limit);
    ...
    if ( !force && cache.isCached(database.address, database.params) ) {
        return callback(undefined, database.address, database.params, cache.getCount(database.address, database.params));
    }

    // Get fresh germplasm records
    _getFreshGermplasm(brapi, database.address, database.params, progress, function(err, germplasm_cache_count, germplasm_term_count) {
        if ( err ) return callback(err);
        let init_cache_index = germplasm_cache_count+1;
        let init_start_index = germplasm_term_count+1;
        _getFreshCrosses(brapi2, database.address, database.params, init_cache_index, init_start_index, progress, function(err, crosses_cache_count, crosses_term_count) {
            return callback(err, database.address, database.params, crosses_cache_count, germplasm_term_count+crosses_term_count);
        });
    });
}
 ...
 * @param  {Function} callback  Callback function(err, cache_count, term_count)
 */
function _getFreshGermplasm(brapi, address, params, progress, callback) {
    ...
    // Progress Function
    const _progress = function(count, total) {
        if ( progress ) {
            progress({
                title: "Getting germplasm entries from the database",
                subtitle: "This will take a few moments...",
    ...
    // Get all of the germplasm
    brapi.get("germplasm", { params, pageSize: 1000 }, _progress).then((data = []) => {
        data.forEach((datum) => {
            ...
            // Unsupported BrAPI version
                ...
                return callback("Unsupported BrAPI Version for /germplasm [version " + JSON.stringify(brapi.version.major) + "]");
            }
            ...
            // Cache db terms once cache chunk size is met
        ...
        // Cache remaining db terms
        cache.put(address, params, cache_index, db_terms, start_index, end_index);

        return callback(undefined, cache_index, end_index);
    }).catch((err) => {
        return callback(`Could not fetch germplasm entries from the database [${err}]`);
    });
}
...
function _getFreshCrosses(brapi, address, params, init_cache_index, init_start_index, progress, callback) {
    ...
    // Get all of the crosses
    brapi.get("crosses", { params, pageSize: 1000 }, _progress).then((data = []) => {
        data.forEach((datum) => {
            ...
            // Unsupported BrAPI version
                ...
                return callback("Unsupported BrAPI Version for /crosses [version " + JSON.stringify(brapi.version.major) + "]");
            }
